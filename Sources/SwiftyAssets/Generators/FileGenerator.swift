...
class FileHeader {
    ...
    func header(filename: String) -> String {
        ...
        let header = [
            ...
            "\tGenerated by \(CLI.name) 🦊 on \(dateFormatter.string(from: Date()))",
...
class FileGenerator {
    ...
    static func generate(atPath path: String, filename: String, fileExtension: Extension, content: String, overwrite: Bool = true) {
        var isDir: ObjCBool = false
        if !FileManager.default.fileExists(atPath: path, isDirectory: &isDir) {
            do {
                try FileManager.default.createDirectory(atPath: path, withIntermediateDirectories: true, attributes: nil)
        ...
        let filePath = "\(path)/\(filename)\(fileExtension.rawValue)"
        guard overwrite == true || !FileManager.default.fileExists(atPath: filePath) else { return }
